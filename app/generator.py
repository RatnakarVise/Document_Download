...
# Global instances for memory efficiency - REMOVED EMBEDDING MODEL TO SAVE MEMORY
_llm_instance = None

def get_llm():
    global _llm_instance
    if _llm_instance is None:
        _llm_instance = ChatGroq(model_name="llama3-70b-8192")
    return _llm_instance

# def generate_fs_from_requirement(requirement: str, fs_template: str) -> str:

# MEMORY-OPTIMIZED VERSIONS - Remove RAG completely to save ~300-400MB
def generate_fs_from_requirement(requirement: str, fs_template: str) -> str:
    """Memory-optimized version without RAG - uses template directly"""
    try:
        prompt_template = ChatPromptTemplate.from_template(
            "You are an SAP Functional Consultant. Strictly use the TEMPLATE structure and headings provided below.\n\n"
            "ABAP CODE:\n{requirement}\n\n"
...
def generate_ts_from_requirement(requirement: str, ts_template: str) -> str:
    """Memory-optimized version without RAG"""
    try:
        prompt_template = ChatPromptTemplate.from_template(
            "You are an SAP Technical Consultant. Strictly use the TEMPLATE structure and headings provided below.\n\n"
            "REQUIREMENT:\n{requirement}\n\n"
...
def generate_abap_code_from_requirement(requirement: str, abap_template: str) -> str:
    """Memory-optimized version without RAG"""
    try:
        prompt_template = ChatPromptTemplate.from_template(
            "You are an expert ABAP developer. Strictly use the TEMPLATE structure and comments provided below as a template.\n\n"
